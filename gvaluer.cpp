...
public:
    ...
    void add_passed_test(int test_num) { passed_set.insert(test_num); }
    ...
    void add_zero_set(std::set<int> &&zs) { zero_sets.emplace_back(zs); }

    bool meet_requirements(const ConfigParser &cfg, const Group *& grp) const;

    void add_total_score() { if (test_score > 0) total_score += test_score; }

    void set_total_score(int total_score) { this->total_score = total_score; }
...
public:
    ...
    void parse_group() {
        ...
        if (find_group(token) != NULL) parse_error(string("group ") + token + " already defined");
        ...
        if (!has_stat_to_judges && global.get_stat_to_judges() >= 0) g.set_stat_to_judges(bool(global.get_stat_to_judges()));
        
        if (!has_stat_to_users && global.get_stat_to_users() >= 0) g.set_stat_to_users(bool(global.get_stat_to_users()));
        groups.push_back(g);
    }

    void parse_groups() {
        while (token == "group") parse_group();
    ...
    void parse(const string &configpath) {
        ...
        if (token != "") parse_error("EOF expected");
    }

    const Group *find_group(const string &id) const {
        for (auto i = groups.begin(); i != groups.end(); ++i) {
            if (i->get_group_id() == id) return &(*i);
        }
        return NULL;
    }
...
void ConfigParser::parse_error(const string &msg) const {
    fprintf(stderr, "%s: %d: %d: parse error: %s\n", path.c_str(), t_line, t_pos, msg.c_str());
    exit(RUN_CHECK_FAILED);
}

void ConfigParser::scan_error(const string &msg) const {
